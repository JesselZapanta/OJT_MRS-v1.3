        ...
        <table id="example" class="table is-striped nowrap" style="width: 100%">
          ...
          @foreach ($interns as $intern)
            <tr>
              ...
              <td><small class="status">{{ $intern->status }}</small></td>
              <td>{{ $intern->institute }}</td>
              <td>{{ $intern->program }}</td>
              <td class="action">
                <div class="action_btn">
                  <a href="{{ route('manageintern.viewprofile', ['intern' => $intern])}}" class="btn view"
                    >View <i class="bx bx-show"></i
                  ></a>
                  <form method="post" action="{{ route('manageintern.deleteintern', ['intern' => $intern]) }}">
                    @csrf
                    @method('delete')
                    <button class="btn danger">Delete <i class="bx bx-trash"></i></button>
                  </form>
                </div>
              </td>
            </tr>
